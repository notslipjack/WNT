...
Android wrapper
Folder: WNT-Android-Wrapper Zip: WNT-Android-Wrapper.zip
...
Put update.json on the default branch (main or master).
Copy .github/workflows/publish-update.yml from the wrapper onto that repo.
Settings > Actions > General > Workflow permissions > Read and write.
Wrapper is baked to check:
https://raw.githubusercontent.com/notslipjack/WNT/main/update.json
If the default branch is master, change main to master.

Each ship: bump versionCode in app/build.gradle, build WNT.apk, create a Release on notslipjack/WNT, attach that file as WNT.apk, put versionCode: 3 in the release body.
The workflow rewrites update.json. The wrapper checks a couple seconds after launch and prompts Update WNT?
